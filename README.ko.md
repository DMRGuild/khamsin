...
운영 setup은 먼저 Cloudflare 인증을 확인하고 필요하면 브라우저 로그인을
실행합니다. 계정을 선택한 뒤(여러 개면 번호로 선택) Worker 이름을 입력하면,
계정의 workers.dev 서브도메인을 조회해
`https://<Worker 이름>.<계정 서브도메인>.workers.dev`를 URL 기본값으로 제안합니다.
Worker를 미리 만들 필요는 없습니다. 사용자 도메인을 직접 입력할 수도 있으며,
기존 사용자 도메인과 명시적인 `--base-url` 값은 유지합니다. 조회에 실패하거나
서브도메인이 등록되지 않았다면 URL을 수동 입력합니다. 터미널 없이 실행할 때는
이 경우 `--base-url`이 필요합니다. `--configure-only`는 URL 조회 없이 오프라인으로
동작합니다.
